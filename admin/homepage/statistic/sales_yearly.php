...
$sql = "SELECT sum(order_details.quantity) as quantity, YEAR(orders.date_buy) as date_buy
        FROM order_details
        INNER JOIN orders ON order_details.order_id = orders.id
        WHERE orders.status = 2
        group by YEAR(orders.date_buy)";
//Chay query
...
<section style="margin: 0 0 0 90px" class="main_content">
    ...
    <script>
        const monthArray = <?php echo json_encode($date)?>;
